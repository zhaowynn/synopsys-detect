...
public class PolarisOperation {
    ...
    public static boolean shouldExecute(DetectToolFilter detectToolFilter, ProductRunData productRunData) {
    ...
    public OperationResult<Void> execute() {
        ToolOperationResultBuilder<Void> resultBuilder = new ToolOperationResultBuilder<>();

        //TODO: Rip event system away from polaris. To maintain being additive, consuming status codes. Ideally no event system required.
        EventSystem fakeEventSystem = new EventSystem();
        fakeEventSystem.registerListener(Event.StatusSummary, resultBuilder::addStatusCode);
        ...
        PolarisTool polarisTool = new PolarisTool(fakeEventSystem, directoryManager, polarisExecutableRunner, detectConfiguration, polarisServerConfig);
