...
public class ShutdownDecider {
    public ShutdownDecision decideShutdown(DetectBootResult detectBootResult) {
        ...
        PhoneHomeManager phoneHomeManager = detectBootResult.getDetectRunData()
                                                .map(DetectRunData::getBlackDuckRunData)
                                                .flatMap(BlackDuckRunData::getPhoneHomeManager)
                                                .orElse(null);

        CleanupDecision cleanupDecision = decideCleanup(detectBootResult.getDetectConfiguration().orElse(null), detectBootResult.getDetectRunData().orElse(null), detectBootResult.getAirGapZip().orElse(null));
        return new ShutdownDecision(phoneHomeManager, diagnosticSystem, cleanupDecision);
    }

    public CleanupDecision decideCleanup(@Nullable PropertyConfiguration detectConfiguration, @Nullable DetectRunData detectRunData, @Nullable File airGapZip) {
        ...
        if (detectRunData != null && detectRunData.getBlackDuckRunData() != null) {
            offline = !detectRunData.getBlackDuckRunData().isOnline();
        }
