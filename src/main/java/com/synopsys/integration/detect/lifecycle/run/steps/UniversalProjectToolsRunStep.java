...
public class UniversalProjectToolsRunStep implements DetectRunStep {
    ...
    private final List<DetectRunStep> toolRunnables;

    public UniversalProjectToolsRunStep(DetectConfigurationFactory detectConfigurationFactory, DirectoryManager directoryManager, EventSystem eventSystem,
        DockerToolRunStep dockerToolRunnable, BazelToolRunStep bazelToolRunnable, DetectorToolRunStep detectorToolRunnable) {
    ...
    public DetectRunState run(DetectRunState previousState) throws DetectUserFriendlyException, IntegrationException {
        ...
        DetectRunState runState = previousState;
        for (DetectRunStep toolRunnable : toolRunnables) {
        ...
        if (runState.isFailure()) {
            return DetectRunState.fail(runResult, runOptions, projectNameVersion);
        } else {
            return DetectRunState.success(runResult, runOptions, projectNameVersion);
        }
    }
}
